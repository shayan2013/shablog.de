(function ($) {
    ...
    function gSlick(gThis) {
        ...
        if (gThis[0].hasAttribute('data-dots')) {
            gss.dots = Boolean(gThis.data('dots'));
        }
        if (gThis[0].hasAttribute('data-arrows')) {
            gss.arrows = Boolean(gThis.data('arrows'));
        }
        if (gThis[0].hasAttribute('data-infinite')) {
            gss.infinite = Boolean(gThis.data('infinite'));
        }
        if (gThis[0].hasAttribute('data-speed')) {
            gss.speed = gThis.data('speed');
        }
        if (gThis[0].hasAttribute('data-slideitemdesktop')) {
            gss.slidesToShow = gThis.data('slideitemdesktop');
        }
        if (gThis[0].hasAttribute('data-slidescroll-desktop')) {
            gss.slidesToScroll = gThis.data('slidescroll-desktop');
        }
        ...
        if (gThis[0].hasAttribute('data-autoplay')) {
            gss.autoplay = gThis.data('autoplay');
        }
        if (gThis[0].hasAttribute('data-autoplayspeed')) {
            gss.autoplaySpeed = gThis.data('autoplayspeed');
        }
        if (gThis[0].hasAttribute('data-fade')) {
            gss.fade = gThis.data('fade');
        }
        if (gThis[0].hasAttribute('data-mode-center')) {
            gss.centerMode = gThis.data('mode-center');
        }
        ...
        if (gThis[0].hasAttribute('data-slideitemtablet')) {
            rTgss.slidesToShow = gThis.data('slideitemtablet');
        }
        if (gThis[0].hasAttribute('data-slidescroll-tablet')) {
            rTgss.slidesToScroll = gThis.data('slidescroll-tablet');
        }
        if (gThis[0].hasAttribute('data-dotstablet')) {
            rTgss.dots = Boolean(gThis.data('dotstablet'));
        }
        if (gThis[0].hasAttribute('data-arrowstablet')) {
            rTgss.arrows = Boolean(gThis.data('arrowstablet'));
        }


        if (gThis[0].hasAttribute('data-slideitemmobile')) {
            rMgss.slidesToShow = gThis.data('slideitemmobile');
        }
        if (gThis[0].hasAttribute('data-slidescroll-mobile')) {
            rMgss.slidesToScroll = gThis.data('slidescroll-mobile');
        }
        if (gThis[0].hasAttribute('data-dotsmobile')) {
            rMgss.dots = Boolean(gThis.data('dotsmobile'));
        }
        if (gThis[0].hasAttribute('data-arrowsmobile')) {
            rMgss.arrows = Boolean(gThis.data('arrowsmobile'));
        }
        let rTSettings = {
                breakpoint: 1024,
                settings: rTgss
    ...
    function gApi(gAB, gParam, gAppend = false) {
        gParam.blockId = gAB.find('.gutentor-post-module-p1').data('gbid');
        ...
        $.ajax({
            ...
            beforeSend: function ( xhr ) {
                ...
                gAB.find('.gutentor-post-module-p1 .grid-container').append(gP4GetLoader(gAB.data('l-ani')));
            },
        }).done (function ( data ) {
            if( !gAppend){
                gAB.find('.gutentor-post-module-p1').replaceWith(data.p1Blog);
            }
            else{
                gAB.find('.gutentor-post-module-p1 .grid-container .grid-row').append($(data.p1Blog).find('.grid-container .grid-row').html());
            ...
            gAB.find('.gutentor-post-module-p1 .grid-container').find('.gutentor-loading-wrap').remove();
    ...
    gDocument.ready(function () {
        ...
        /* Module Accordion*/
        gDocument.on('click', '.gutentor-accordion-item-heading', function (e) {

            let gThis = $(this);
            let accordion_parent = gThis.parent();
            gThis.toggleClass('gutentor-accordion-active');
            if(gThis.hasClass('gutentor-accordion-active')){
                accordion_parent.siblings().find('.gutentor-accordion-item-heading').removeClass('gutentor-accordion-active');
            }
            e.preventDefault();
        });
