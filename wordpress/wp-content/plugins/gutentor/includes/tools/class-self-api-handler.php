...
if ( ! class_exists( 'Gutentor_Self_Api_Handler' ) ) {
	...
	class Gutentor_Self_Api_Handler extends WP_Rest_Controller {
		...
		public function max_num_pages( \WP_REST_Request $request ) {
			...
			if ( $request->get_param( 'offset' ) && ! ( $query_args > 1 ) ) {
		...
		public function gadvancedb( \WP_REST_Request $request ) {
			...
			if ( $paged ) {
				...
				if ( has_blocks( $post->post_content ) ) {
					...
					$p1Block = gutentor_get_block_by_id( $blocks, $blockId );

					/*Get Default Attributes*/
					$p1_attr          = Gutentor_P1_Blog::get_instance()->get_attrs();
					$common_attr      = gutentor_block_base()->get_common_attrs();
					$default_pre_attr = array_merge( $p1_attr, $common_attr );
					$default_attr     = array();
					...
					$final_attrs          = array_merge( $default_attr, $p1Block['attrs'] );
					$final_attrs['paged'] = $paged;
					if ( $taxonomy != 'gAll' && $term != 'gAll' ) {
					...
					$response['p1Blog'] = Gutentor_P1_Blog::get_instance()->render_callback( $final_attrs, '' );

					/*
					max num of pages*/
					// the query
					$query_args = array(
						'posts_per_page' => $final_attrs['postsToShow'],
						'orderby'        => $final_attrs['orderBy'],
						'order'          => $final_attrs['order'],
					);
					if ( isset( $final_attrs['pTaxType'] ) && ! empty( $final_attrs['pTaxType'] ) &&
						...
						if ( is_array( $final_attrs['pTaxTerm'] ) ) {
							$p1_terms = array();
							foreach ( $final_attrs['pTaxTerm'] as $p1_term ) {
