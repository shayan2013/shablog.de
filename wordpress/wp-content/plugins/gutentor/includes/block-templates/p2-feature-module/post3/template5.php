...
if ( ! class_exists( 'Gutentor_P2_Post3_Template5' ) ) {

	/**
	 * Gutentor_P2_Post3_Template5 Class For Gutentor
	 *
	 * @package Gutentor
	 * @since 2.0.0
	 */
	class Gutentor_P2_Post3_Template5 extends Gutentor_Query_Elements {
		...
		public function template_data( $output, $the_query, $attributes ) {
			...
			while ( $the_query->have_posts() ) :
				$the_query->the_post();
				if ( $index === 0 ) {
					$output .= "<div class='" . apply_filters( 'gutentor_post_module_p2_grid_class', 'grid-lg-12 grid-lg-12 grid-12', $attributes ) . "'>";
					$output .= $this->p2_single_article( get_post(), $attributes, $index );
					$output .= '</div>';
				}
				if ( $index === 1 || $index === 2 ) {
					$output .= "<div class='" . apply_filters( 'gutentor_post_module_p2_grid_class', 'grid-lg-6 grid-lg-6 grid-12', $attributes ) . "'>";
					$output .= $this->p2_single_article( get_post(), $attributes, $index );
					$output .= '</div>';
				}
				$index++;
...
Gutentor_P2_Post3_Template5::get_instance()->run();
